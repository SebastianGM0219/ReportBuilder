...
export const runQuery = createAsyncThunk("query/run", async (queryInfo) => {
  try {
...
export const pivot = createAsyncThunk("query/pivot", async (pivotInfo) => {
  try {
...
export const querySlice = createSlice({
  ...
  reducers: {
    ...
    resetBasedOnCommand: (state, action) => {
      ...
      const fromRegex =
        /FROM(.*?)(LEFT JOIN|INNER JOIN|RIGHT JOIN|FULL JOIN|WHERE|ORDER BY|;)/i;
      ...
      const joinRegex =
        /(LEFT JOIN|INNER JOIN|RIGHT JOIN|FULL JOIN)(.*?)(?=(?:WHERE|ORDER BY|LEFT JOIN|INNER JOIN|RIGHT JOIN|FULL JOIN|;)|$)/gi;
      ...
      for (let i = 0; i < joinConditions.length; i++) {
        ...
        let tempOperator = [],
          tempRField = [],
          tempLField = [];
        const joinConditionRegex =
          /(?:LEFT JOIN|INNER JOIN|RIGHT JOIN|FULL JOIN)\s+(\w+)\s+ON\s+(.*)/i;
        ...
        // const logicalOperatorsRegex = /AND|OR/gi;
        ...
        if (
          state.relationFields[state.relationFields.length - 1].RTable
            .length === 0
        ) {
          state.relationFields = [
            ...state.relationFields.slice(1),
            newRelationField,
          ];
        } else {
    ...
    AddRelationData: (state, action) => {
      ...
      if (
        state.relationFields[state.relationFields.length - 1].RTable.length ===
        0
      ) {
        const newRelationField = {
          LTable: leftTable,
    ...
    removeFieldCalcDrop: (state, action) => {
      ...
      //      let removedIndex = -1;
      const newState = state.fieldCalcDrop.filter(
        (item) =>
          (item.filterVariant && item.filterVariant.length === 0) ||
          (item.filterVariant &&
            item.filterVariant[0] &&
            item.filterVariant[0].id !== id)
      );
    ...
    removeFilter: (state, action) => {
      ...
      const newState = state.filterFields.filter((item, index) => {
        if (
          index !== state.filterFields.length - 1 &&
          item.filterVariant[0].id === id
        )
          removedIndex = index;
        if (
      ...
      if (newState.length !== state.filterFields.length) {
        ...
        if (andOrArray !== null && andOrArray.length > 1) {
          ...
          const removedArray = andOrArray
            .slice(0, removedIndex - 1)
            .concat(andOrArray.slice(removedIndex - 0));
    ...
    removeSelector: (state, action) => {
      ...
      const newState_filter = state.selectFields
        .filter((item) => item.id !== id)
        .map((item) => item.data.table);
      const uniqueArray = [...new Set(newState_filter)];
      const relation = state.relationFields.filter(
        (item) =>
          uniqueArray.includes(item.RTable[0]) &&
          uniqueArray.includes(item.LTable[0])
      );
...
*/
      ...
      const updatedFields = state.selectFields.map((item) => {
        if (item.id === id) {
          return {
            ...item,
            data: { ...item.data, header_name: name, aggreType: aggreType },
          };
        }
        return item;
      });
...
//      const newState = state.selectFields.filter(item => item.id !== id);
      ...
      state.selectFields = newState;
*/
      const { id, name, field, command, dropbox, columnId, table, type } =
        action.payload;
    ...
    setSourceColumnSelector: (state, action) => {
      ...
      const { source, sourceColumn, field, type } = action.payload;
      const updatedFields = state.selectFields.map((item) => {
        if (item.id === state.clickField.id) {
          return {
            ...item,
            data: { ...item.data, table: source, field: field, type: type },
            text: sourceColumn,
          };
        }
        return item;
    ...
    clickSelector: (state, action) => {
      ...
      if (
        state.clickField &&
        state.clickField.data &&
        action.payload.id.includes("function")
      ) {
